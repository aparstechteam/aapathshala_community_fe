...
export const RtxEditor: React.FC<EditorProps> = ({ content = "", onUpdate }) => {
    ...
    return (
        <div className="grid gap-1">
            <div className="mb-2 flex gap-2 text-ice">
                <button className="bg-ice/20 hover:bg-ice/40 duration-200 focus:!bg-ice focus:text-white w-8 px-2 py-0.5 rounded-lg font-bold" type="button"
                    onClick={() => editor?.chain().focus().toggleBold().run()} disabled={!editor?.can().chain().focus().toggleBold().run()}>
                    B
                </button>
                <button className="bg-ice/20 hover:bg-ice/40 w-8 duration-200 focus:!bg-ice focus:text-white font-semibold px-2 py-0.5 rounded-lg italic" type="button"
                    onClick={() => editor?.chain().focus().toggleItalic().run()} disabled={!editor?.can().chain().focus().toggleItalic().run()}>
                    i
                </button>
                <button className="bg-ice/20 hover:bg-ice/40 duration-200 focus:!bg-ice focus:text-white w-8 px-2 py-0.5 rounded-lg underline" type="button"
            ...
            <EditorContent className="tiptap border-0 relative z-[9] tiptap max-h-[200px] overflow-y-auto aa focus:!outline-none !outline-none focus:!border-0 ring-2 ring-ash focus-within:ring-olive/50 focus:ring-2 focus-within:!ring-2 !rounded-lg" editor={editor} />
        </div>
    );
};
