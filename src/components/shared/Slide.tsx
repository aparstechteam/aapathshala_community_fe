        ...
        <div className="max-w-[90vw] lg:max-w-[calc(100vw-56vw)]">
                ...
                <CarouselContent className="-ml-1">
                    {posts.slice(0, 5).map((x, index) => (
                        <CarouselItem
                            ...
                            key={index} className={cn("pl-1 relative cursor-pointer hover:scale-95 transition-all duration-300", posts.length === 1 && "lg:basis-full", posts.length > 1 && "lg:basis-1/2")}>
                                ...
                                <div className='grid gap-2 h-full w-full max-w-[350px]'>
                                    ...
                                    <div className='flex gap-2 items-center p-1'>
                                        ...
                                        <div className="flex justify-between items-start gap-2 w-full">
                                            <div>
                                                <h2 className='text-base font-medium text-gray-600 dark:text-gray-200'>{x.user?.name}

                                                    <span className="scale-90">
                                                        <Tagtag tags={x.tags} />
                                                    </span>
                                                </h2>
                                                <h2 className='text-xs font-light text-gray-500 dark:text-gray-200'>{fromNow(new Date(x.createdAt))}</h2>
                                            </div>
                                            {user?.role === 'ADMIN' && (
                                                <button type="button"
                                                    onClick={(e) => {
                                    ...
                                    <div className="flex px-4 md:px-0 justify-between items-center gap-2">
                                        <div className="flex items-center w-full justify-between gap-5">
                                            ...
                                            <div className="rounded-md gap-2 flex justify-center items-center text-sm font-medium shadow-none !text-gray-900 dark:!text-white">
                                                ...
                                                <span className="pt-1">{x?.reactions.length || 0} রিয়েক্ট</span>
                                            </div>


                                            <Link className="rounded-md flex gap-2 justify-center items-center text-sm font-medium shadow-none !transition-opacity !text-gray-900 dark:!text-white" href={`/post/${x.id}`}>
                                                {" "}
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M18.3332 9.63901C18.3332 14.0416 14.6017 17.6113 9.99984 17.6113C9.45874 17.612 8.91921 17.5619 8.38768 17.4621C8.00511 17.3903 7.81382 17.3544 7.68028 17.3748C7.54673 17.3951 7.35749 17.4958 6.97899 17.6971C5.90828 18.2665 4.6598 18.4676 3.4591 18.2442C3.91545 17.6829 4.22712 17.0094 4.36465 16.2874C4.44799 15.8458 4.2415 15.4167 3.93224 15.1027C2.52762 13.6764 1.6665 11.7543 1.6665 9.63901C1.6665 5.23639 5.39799 1.66675 9.99984 1.66675C14.6017 1.66675 18.3332 5.23639 18.3332 9.63901Z" stroke="#575757" strokeWidth="1.25" strokeLinejoin="round" />
                                                    <path d="M9.9961 10H10.0036M13.3257 10H13.3332M6.6665 10H6.67398" stroke="#575757" strokeWidth="1.66667" strokeLinecap="round" strokeLinejoin="round" />
                                                </svg>
                                                <span className="pt-1">{x.commentCount} কমেন্ট</span>
                                            </Link>

                                            <button type="button" className="rounded-md flex gap-2 items-center shadow-none duration-300 !transition-opacity !text-gray-900 dark:!text-white">
                                                {" "}
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M17.5399 2.54402C15.7249 0.589388 2.07223 5.37758 2.0835 7.12575C2.09628 9.10814 7.41523 9.71801 8.88952 10.1316C9.77607 10.3804 10.0135 10.6353 10.2179 11.565C11.1438 15.7753 11.6086 17.8695 12.668 17.9163C14.3566 17.9909 19.3113 4.45159 17.5399 2.54402Z" stroke="#575757" strokeWidth="1.25" />
                                                    <path d="M9.5835 10.4167L12.5002 7.5" stroke="#575757" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" />
                                                </svg>
            ...
            <div className="py-3 flex justify-center items-center gap-2">
                {Array.from({ length: count }).map((_, index) => (
                    <div key={index} className={cn("w-2 h-2 duration-300 bg-gray-300 dark:bg-ash rounded-full", current === index + 1 && "bg-green-500")}></div>
                ))}
            </div>
        </div>
    )
