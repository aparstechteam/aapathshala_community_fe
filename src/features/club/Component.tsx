...
export const ClubComponent = (props: Props) => {
    ...
    const fetchPinnedPosts = async () => {
      try {
        const response = await axios.get(
          `${secondaryAPI}/api/post/pinned_posts?group_id=${club?.id}`,
          {
            headers: {
  ...
  async function handlejoin(e: FormEvent) {
    ...
    try {
      ...
      const res = await axios.post(
        `${secondaryAPI}/api/group/${club?.id}/join`,
        {
          group_id: club?.id,
        },
        {
  ...
  async function handleLeave(e: FormEvent) {
    ...
    try {
      const res = await axios.post(
        `${secondaryAPI}/api/group/${club?.id}/leave`,
        {
          group_id: club?.id,
        },
        {
    ...
    <div className="w-full min-h-screen bg-[#F5F6F7] dark:bg-[#171717] z-0 text-black dark:text-white">
      <div className="max-w-screen-2xl gap-4 px-0 lg:px-4 w-full mx-auto flex">
            ...
            <div className="w-[320px] bg-white dark:bg-gray-600/20 rounded-xl ring-1 ring-ash dark:ring-ash/20 mt-4 p-3">
              ...
              {courses?.map((g) => (
                <div
                  key={g.group_id}
                  className={cn(
                    g?.slug === club?.slug && "bg-elegant/10 !text-elegant",
                ...
                >
                  <div className="flex items-center gap-4">
                    <Avatar className="!rounded-xl">
                      ...
                      <AvatarFallback className="!rounded-xl dark:bg-gray-700/20 bg-elegant/20 pt-1">
                        ME
                      </AvatarFallback>
                    </Avatar>
        ...
        <div className="w-full pt-0 lg:pt-4">
          ...
          <div
            className="relative h-[200px] md:h-[300px] w-full bg-ice/20"
          ...
          <div className="py-3 grid w-full lg:grid-cols-2 justify-start bg-white dark:bg-gray-600/20 lg:rounded-b-xl px-3 shadow-sm lg:justify-between">
            <div className="grid gap-1 w-full">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-sm font-medium hover:text-hot text-light dark:text-gray-400 flex items-center gap-2"
                >
                  <svg
                    width="16"
                    height="16"
                ...
                >
                  ...
                  <span className="pt-1">{club?.member_count} Members </span>
                </button>
              </div>
            </div>
            ...
            <div className="flex w-full items-center justify-center py-2 lg:py-0 lg:justify-end gap-2">
              <Button
                ...
                size="sm"
                className="bg-hot text-white hover:text-hot ring-1 ring-ash hover:bg-hot/20 duration-300 transition-all hover:!bg-opacity-80"
              >
            ...
            <div className="w-full space-y-2">
              ...
              {tab === "posts" && (
                <>
                  {club?.is_member && (
                    <CreatePost
                      group_type={club?.type}
                      group_id={club?.id as string}
                      subject_id={club?.data?.subject as string}
                    />
                  )}
              ...
              {tab === "homework" && <Whiteboard image={"/whiteboard.png"} />}
            </div>
