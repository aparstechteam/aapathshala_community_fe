...
export const CommentComponent = ({ comment, authorId, setSuccess, parentCommentId }: CommentProps) => {
    ...
    const reactToComment = async (r: string) => {
        ...
        try {
            ...
            const res = await axios.post(`${secondaryAPI}/api/post/${comment.post_id}/comments/${comment.id}/react`, {
                authorId,
                reactionType: r,
            }, {
                headers: {
                            ...
                            <h2 className="font-bold flex items-center gap-1 text-xs lg:text-sm">
                                ...
                                {comment.user_id === curicity_id && (
                                    <svg width="36" height="17" viewBox="0 0 36 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <path d="M8.67442 8.15112C8.83754 8.15112 8.97888 8.26417 9.01473 8.4233L9.19371 9.21784C9.31592 9.76037 9.73959 10.184 10.2821 10.3063L11.0767 10.4852C11.2358 10.5211 11.3488 10.6624 11.3488 10.8255C11.3488 10.9887 11.2358 11.13 11.0767 11.1659L10.2821 11.3448C9.73959 11.467 9.31592 11.8907 9.19371 12.4332L9.01473 13.2278C8.97888 13.3869 8.83754 13.5 8.67442 13.5C8.5113 13.5 8.36996 13.3869 8.33411 13.2278L8.15513 12.4332C8.03292 11.8907 7.60925 11.467 7.06672 11.3448L6.27218 11.1659C6.11304 11.13 6 10.9887 6 10.8255C6 10.6624 6.11304 10.5211 6.27218 10.4852L7.06672 10.3063C7.60925 10.184 8.03292 9.76037 8.15513 9.21785L8.33411 8.4233C8.36996 8.26417 8.5113 8.15112 8.67442 8.15112Z" fill="white" />
                                        <path d="M21 9.08L20.892 9.428H23.172L23.064 9.068C22.792 8.204 22.568 7.48 22.392 6.896C22.224 6.304 22.12 5.932 22.08 5.78L22.032 5.54C21.96 5.94 21.616 7.12 21 9.08ZM19.92 12.5H18.336L21.072 4.412H23.028L25.764 12.5H24.132L23.556 10.664H20.496L19.92 12.5ZM28.5339 12.5H27.0219V4.412H28.5339V12.5Z" fill="white" />
                                    </svg>
                                )}
                            </h2>
                            <p className="text-sm text-gray-400 dark:text-gray-400">{fromNow(comment?.created_at as Date)}</p>
                        </div>

                        <div>
                            <div className="mb-2 select-text">
                                <div className='px-2 w-full'>
                                    <AppMath className='!max-w-[calc(100vw-180px)] lg:!max-w-xl' formula={comment?.content || ''} key={key} />
                                </div>
                            </div>
                    ...
                    <div className="flex items-center px-2 mt-2 space-x-4 text-xs md:text-sm font-medium text-light">
                        ...
                        {comment.user_id === curicity_id ? (
                            <>
                                ...
                                <button type='button' className={`flex items-center gap-1 ${reaction === 'dissatisfied' ? 'text-[#F43F5E] font-bold' : ''}`}
                                    onClick={() => reactToComment('dissatisfied')}>
                                    <svg width="19" height="18" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path fillRule="evenodd" clipRule="evenodd" d="M8.39664 2.18617C7.17914 1.56307 5.47398 1.37219 3.60867 2.51637C2.06274 3.46464 1.19287 5.44545 1.49811 7.72134C1.80482 10.0082 3.28501 12.5947 6.41971 14.9149C7.50825 15.7211 8.30716 16.3127 9.49971 16.3127C10.6923 16.3127 11.4912 15.7211 12.5797 14.9149C15.7144 12.5947 17.1946 10.0082 17.5013 7.72134C17.8065 5.44545 16.9367 3.46464 15.3907 2.51637C13.155 1.14497 11.1493 1.69153 9.93769 2.60146C9.92223 2.61307 9.90714 2.6244 9.89242 2.63544L11.8749 6.49285C11.9953 6.72709 11.937 7.0135 11.7347 7.1821L9.95787 8.66277L11.0223 9.72723C11.2048 9.90974 11.2397 10.193 11.1069 10.4144L9.98191 12.2894C9.82208 12.5558 9.47656 12.6421 9.21017 12.4823C8.94378 12.3225 8.8574 11.977 9.01724 11.7106L9.91628 10.2122L8.72683 9.02272C8.61508 8.91097 8.55549 8.75736 8.56265 8.59948C8.56981 8.4416 8.64306 8.29402 8.76447 8.19285L10.6683 6.60632L8.39664 2.18617Z" fill="#FC465D" />
                                    </svg>
                                    <span className=''>No</span>
                                    <p className='flex items-center'>
                                        <span className={cn('ring-1 min-w-7 pt-0.5 rounded-full text-xs', reaction === 'dissatisfied' ? 'text-hot bg-hot/10 ring-hot/50' : 'bg-light/10 text-light ring-light/50')}>
                                            {comment.dissatisfied_count}
                                        </span>
                                    </p>
                                </button>
                                <button type='button' className="flex items-center" onClick={() => setReplyParent()}>
                                    {/* <ReplyIcon size={16} /> */}
                                    <span className=''>
                                        Reply
                                    </span>
                            ...
                            <>
                                <button disabled type='button' className={`flex items-center gap-1 ${reaction === 'satisfied' ? 'text-elegant font-bold' : ''}`}
                                    onClick={() => reactToComment('satisfied')}
                                >
                                    <span className=''>Satisfied</span>
                                    <p className='flex items-center gap-1'>
                        ...
                        {user.role === 'ADMIN' ? (
                            ...
                            <div className='flex items-center gap-2 justify-end w-full'>
                                <button type='button' className="flex items-center text-red-500 hover:underline"
                                    onClick={() => deleteComment(comment.id)}
                                >
                                    <TrashIcon size={16} className="mr-1 text-red-500" />
                                    <span className='pt-1'>
            ...
            {comment.replies && comment.replies.length > 0 && (
                <div className="ml-10 grid">
                    {comment.replies.map((reply) => (
                        <CommentComponent key={reply.id} comment={reply} authorId={authorId} parentCommentId={parentCommentId} />
                    ))}
                </div>
            )}
