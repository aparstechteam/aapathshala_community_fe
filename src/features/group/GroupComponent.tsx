...
import { Memberr, MembersComponent } from "./Members";
...
export const GroupComponent = (props: Props) => {
  const { group, id, groups } = props;
  ...
  async function handlejoin(e: FormEvent) {
    ...
    try {
      ...
      const res = await axios.post(
        `${secondaryAPI}/api/group/${group?.id}/join`,
        {
          group_id: group?.id,
        },
        {
  ...
  async function handleLeave(e: FormEvent) {
    ...
    try {
      const res = await axios.post(
        `${secondaryAPI}/api/group/${group?.id}/leave`,
        {
          group_id: group?.id,
        },
        {
  ...
  const joining_rules = (
    <Dialog open={showJoiningRules} onOpenChange={setShowJoiningRules}>
      <DialogContent className="sm:max-w-[425px] bg-white text-light dark:text-white dark:bg-light/20 p-4">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-center gap-2 text-center text-black dark:text-white py-2 ">
            গ্রুপে জয়েন করার শর্তাবলী
          </DialogTitle>
        </DialogHeader>
        <div className="grid gap-4 !text-black dark:!text-white pb-4">
          <p>
            <span className="font-bold"> ১. ⁠সদাচরণ বজায় রাখা: </span>
            প্রত্যেক সদস্যের প্রতি শ্রদ্ধাশীল আচরণ করতে হবে। ব্যক্তিগত আক্রমণ,
            অপমানজনক ভাষা বা নেতিবাচক মন্তব্য করা নিষিদ্ধ।
          </p>
          <p>
            <span className="font-bold"> ২. শেখার পরিবেশ বজায় রাখা: </span>
            ক্লাস এবং আলোচনায় Learning Mindset রাখতে হবে। কেউ কোনো কিছু না জানলে
            তিরষ্কার করা যাবে না৷
          </p>

          <p>
            <span className="font-bold"> ৩. ⁠সময়নিষ্ঠতা: </span>
            ক্লাবের সেশন, ওয়ার্কশপ এবং অন্যান্য কার্যক্রমে সময়মতো উপস্থিত হতে
            হবে। এর মাধ্যমে তোমরা পেয়ে যেতে পারো এক্সাইটিং গিফটস।
          </p>

          <p>
            <span className="font-bold"> ৪. ⁠সহযোগিতামূলক মনোভাব: </span>
            নতুন সদস্যদের সহায়তা করা। পড়াশোনা বা এই সম্পর্কিত যেকোনো প্রশ্ন করা
            যাবে৷
          </p>
          <p>
            <span className="font-bold">
              {" "}
              ৫. ⁠পড়াশোনার ম্যাটেরিয়াল এর সুরক্ষা:{" "}
            </span>
            কমিউনিটি থেকে পাওয়া কন্টেন্ট রিসোর্সেস যথাযথভাবে ব্যবহার করতে হবে।
          </p>

          <p className="text-center italic text-light dark:text-white">
            উপরের শর্তাবলীর সাথে একমত হলে গ্রুপে জয়েন করো
          </p>
        </div>
        <DialogFooter>
          <Button
            className="!bg-light !rounded-lg !w-full !text-white dark:!bg-gray-600/20"
            variant="outline"
            onClick={() => {
      ...
      try {
        if (slug) {
          const response = await axios.get(
            `${secondaryAPI}/api/group/${slug}/members?page=${page}&limit=20&search=${dSearchQuery}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
              },
  ...
  return (
    <div className="w-full min-h-screen bg-[#F5F6F7] dark:bg-[#171717] z-0 mt-1 pt-10 lg:pt-0 text-black dark:text-white">
      {/* <ReactConfetti /> */}
      <div className="max-w-7xl gap-4 px-0 lg:px-4 w-full mx-auto flex">
        {leaveDialog}
        {joining_rules}
        {/* Joined groups  */}
        <div className={cn(groups?.length > 0 ? "hidden lg:block" : "hidden")}>
          <div className="space-y-4 min-h-[300px] w-[320px] bg-white dark:bg-gray-600/20 rounded-xl ring-1 ring-ash dark:ring-ash/20 mt-4 p-4">
            <h2 className="text-base font-semibold py-3">
              আমার গ্রুপ এবং ক্লাব সমূহ
            </h2>
            {groups?.map((g) => (
              <div
                key={g.id}
              ...
              >
                <div className="flex items-center gap-4">
                  <Avatar className="!rounded-xl">
                    ...
                    <AvatarFallback className="!rounded-xl dark:bg-gray-700/20 bg-rose-100 pt-1">
                      ME
                    </AvatarFallback>
                  </Avatar>
          ...
          <div className="relative h-[200px] md:h-[300px] w-full">
            {!!group?.image ? (
              <Image
                ...
                alt="cover-image"
                fill
                className="!z-[0] object-contain object-center lg:rounded-t-xl bg-[blue] backdrop-blur-sm bg-opacity-20"
              />
            ) : (
              <Image
                src={group?.cover as string}
                alt="cover-image"
                fill
                className="!z-[0] object-cover object-center lg:rounded-t-xl bg-[blue] backdrop-blur-sm bg-opacity-20"
              />
            )}
          </div>
          ...
          <div className="py-3 grid w-full lg:grid-cols-2 justify-start bg-white dark:bg-gray-600/20 lg:rounded-b-xl px-3 shadow-sm lg:justify-between">
            <div className="grid gap-1 w-full">
              <h1 className="text-[20px] text-start justify-start flex items-center gap-2 font-extrabold text-black dark:text-white">
                <span className="pt-1">{group?.name}</span>
              </h1>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setShowMembers(true)}
                  className="text-sm font-medium text-light dark:text-gray-400 flex items-center gap-2"
                >
                  <svg
                    width="16"
                    height="16"
                ...
                >
                  ...
                  <span className="pt-1">{group?.member_count} Members </span>
                </button>
              </div>
            </div>
            ...
            <div className="flex w-full items-center justify-center py-2 lg:py-0 lg:justify-end gap-2">
              <Button
                ...
                size="sm"
                className="dark:!bg-rose-700 ring-1 ring-ash hover:bg-gray-200 duration-300 transition-all hover:!bg-opacity-80"
              >
                {isJoined ? (
                  <Check size={13} />
                ) : loading ? (
                ...
                <span className="">{isJoined ? "সদস্য" : "জয়েন করো"}</span>
              </Button>
              <Button
                size="sm"
                onClick={() => {
              ...
              >
                ...
                <span className="">ইনভাইট করো</span>
              </Button>
              {isJoined && (
                <Button
                  type="button"
                  ...
                  size="sm"
                  className="dark:!bg-rose-700 ring-1 ring-ash hover:bg-red-100 duration-300 transition-all hover:!bg-opacity-80"
                >
                  <svg
                    width="13"
                    height="12"
                ...
                <>
                  {group?.is_member && (
                    <CreatePost
                      ...
                      subject_id={group?.data?.subject as string}
                    />
                  )}

                  {!!group?.id && <NewsFeed groupId={group?.id as string} />}
                </>
              )}
            </div>
