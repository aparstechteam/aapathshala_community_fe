import { Avatar, AvatarFallback, AvatarImage, Badge, Button, copyLink, DialogContent, DialogFooter, DialogHeader, DialogTitle, useDebounce, useUser } from '@/components'
...
export const GroupComponent = (props: Props) => {
            ...
            <DialogContent className="sm:max-w-[425px] bg-white text-light dark:text-white dark:bg-light/20 p-4">
                ...
                <DialogFooter>
                    <div className="grid grid-cols-2 !justify-between gap-3">
                        ...
                        <Button className='!bg-hot !rounded-lg !w-full !text-white dark:!bg-gray-600/20'
                            variant="outline"
                            onClick={(e) => {
                                handleLeave(e)
                            }}
    ...
    async function handlejoin(e: FormEvent) {
        ...
        try {
            setLoading(true)
            const res = await axios.post(`${secondaryAPI}/api/group/${group?.id}/join`, {
                group_id: group?.id
            }, {
                headers: {
    ...
    async function handleLeave(e: FormEvent) {
        e.preventDefault()
        try {
            const res = await axios.post(`${secondaryAPI}/api/group/${group?.id}/leave`, {
                group_id: group?.id
            }, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('accessToken')}`
            ...
            <div className='max-w-7xl gap-4 px-0 lg:px-4 w-full mx-auto flex'>
                ...
                <div className={cn(groups?.length > 0 ? 'hidden lg:block' : 'hidden')}>
                    <div className="space-y-4 min-h-[300px] w-[320px] bg-white dark:bg-gray-600/20 rounded-xl ring-1 ring-ash dark:ring-ash/20 mt-4 p-4">
                        <h2 className='text-base font-semibold py-3'>আমার গ্রুপ এবং ক্লাব সমূহ</h2>
                        {groups?.map((g) => (
                            <div
                                key={g.id}
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    <Avatar className="!rounded-xl">
                                        ...
                                        <AvatarFallback className="!rounded-xl dark:bg-gray-700/20 bg-green-100 pt-1">ME</AvatarFallback>
                                    </Avatar>
                                    <div className="space-y-1">
                                        <Link href={`/group/${g.slug}`} className="font-medium text-sm leading-none">{g.name}</Link>
                ...
                <div className='w-full pt-0 lg:pt-4'>
                    ...
                    <div className='relative h-[200px] md:h-[300px] w-full'>
                        {!!group?.image ? (
                            <Image src={group?.image} alt='cover-image' fill className='!z-[0] object-contain object-center lg:rounded-t-xl bg-[blue] backdrop-blur-sm bg-opacity-20' />
                        ) : (
                            <Image src={group?.cover as string} alt='cover-image' fill className='!z-[0] object-cover object-center lg:rounded-t-xl bg-[blue] backdrop-blur-sm bg-opacity-20' />
                        )}
                    </div>
                    ...
                    <div className='py-3 grid w-full lg:grid-cols-2 justify-start bg-white dark:bg-gray-600/20 lg:rounded-b-xl px-3 shadow-sm lg:justify-between'>
                        <div className='grid gap-1 w-full'>
                            ...
                            <div className='flex items-center gap-2'>
                                <button onClick={() => setShowMembers(true)} className='text-sm font-medium text-light dark:text-gray-400 flex items-center gap-2'>
                                    ...
                                    <span className='pt-1'>
                                        {group?.visibility === 'PUBLIC' ? "পাবলিক গ্রুপ" : "প্রাইভেট গ্রুপ"}
                                    </span>
                                </button>
                                <button onClick={() => setShowMembers(true)} className='text-sm font-medium text-light dark:text-gray-400 flex items-center gap-2'>
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className='flex w-full items-center justify-center py-2 lg:py-0 lg:justify-end gap-2'>
                            <Button disabled={isJoined} type='button' onClick={(e) => {
                                e.preventDefault()
                                setShowJoiningRules(true)
                            }} size='sm' className='dark:!bg-green-700 ring-1 ring-ash hover:bg-gray-200 duration-300 transition-all hover:!bg-opacity-80'>
                                {isJoined ?
                                    <Check size={13} /> : loading ?
                    ...
                    <div className='flex gap-4'>
                        <div className='w-full space-y-2'>
                            {showMembers ? (
                                ...
                                <>
                                    {group?.is_member && (
                                        <CreatePost group_type={group?.type} group_id={group?.id as string}
                                            subject_id={group?.data?.subject as string} />
                                    )}

                                    {!!group?.id && (
                                        <NewsFeed groupId={group?.id as string} />
                                    )}
                                </>
                            )}
                        </div>
