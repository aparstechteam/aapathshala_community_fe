...
export function QuizComponent(props: { post_id: string }) {
  ...
  const handleOptionSelect = (
    ...
    setAnswered((prev) => {
      const existingAnswerIndex = answered.findIndex(
        (a) => a.qid === quizData[questionIndex].id.toString()
      );
      if (existingAnswerIndex !== -1) {
        ...
        return [
          ...prev,
          { qid: quizData[questionIndex].id.toString(), optn: isCorrect },
        ];
      }
    });
    ...
    async function getQuiz() {
      ...
      try {
        ...
        const response = await axios.get(
          `${secondaryAPI}/api/post/${post_id}/quiz`,
          {
  ...
  useEffect(() => {
    async function getQuiz() {
      try {
        setLoading(true);
        if (post_id) {
          const res = await axios.get<QuizResult>(
            `${secondaryAPI}/api/post/${post_id}/quiz/results`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
              },
  ...
  const handleSubmitExam = async () => {
    try {
      await axios.post(
        `${secondaryAPI}/api/post/${post_id}/quiz/submit`,
        {
          score: score,
          duration: ftime(time),
        },
  ...
  const endQuizDialog = (
    ...
    >
      <DialogOverlay className="!bg-gradientje bg-opacity-20 z-[99] dark:!bg-gray-800/80"></DialogOverlay>
      <DialogContent className="p-4 z-[100] w-full max-w-xl bg-white dark:bg-gray-900">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            Your quiz performance and score
          </DialogDescription>
        </DialogHeader>
        <div className="dark:text-white text-gray-900 p-8 flex flex-col items-center gap-6">
          ...
          <div className="flex items-center gap-3 bg-gradientje px-6 py-2 rounded-full">
            ...
            <span className="font-semibold text-white text-lg pt-1">
              Your Score: {score}
            </span>
          </div>
        </div>
        <DialogFooter>
          <DialogClose asChild>
            <Button
              type="button"
              className="w-full !rounded-full !bg-gradient-to-r from-rose-400 via-rose-800 to-rose-500 text-white pb-1"
              onClick={() => {
                setEndExam(false);
              }}
            >
  ...
  const quizQuestion = (
    ...
    >
      ...
      <DialogContent className="bg-white w-full max-w-2xl dark:bg-gray-900">
        <DialogHeader>
          <DialogTitle className="pt-4 px-4 text-center text-black dark:text-white">
            Quiz Question
          </DialogTitle>
        </DialogHeader>
        <div className="w-full mx-auto grid gap-4">
          <div className="flex px-4 items-center justify-between text-base font-medium text-black dark:text-white">
            ...
            <h2 className="pt-3 pb-2 px-4 rounded-lg ring-1 ring-ash bg-white dark:bg-gray-900">
              Time Remaining: {ftime(time)}
            </h2>
          </div>
          <ScrollArea className="h-[calc(100vh-10rem)] p-2 lg:p-4">
            {error && (
              <div className="text-center text-white bg-rose-900/30 p-4 rounded-lg">
                {error}
              </div>
            )}
            <div className="space-y-2">
              {quizData?.map((mcq, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="bg-gray-50 text-gray-700 dark:bg-gray-900 dark:text-gray-100 shadow-md rounded-lg p-2 lg:p-4 space-y-4"
                  >
                    ...
                    <div className="space-y-2">
                      {mcq.options.map((option, optionIndex) => {
                        ...
                        let bgColor = "bg-rose-500 bg-opacity-10";
                        if (isSelected && showResult) {
                          bgColor = isCorrect
                            ? "dark:bg-rose-500/40 bg-rose-200"
                            : "dark:bg-red-500/40 bg-red-200";
                        } else if (isSelected && !showResult) {
                          bgColor = "bg-rose-500/30 ring-rose-500 ring-2";
                        } else if (!isSelected && showResult && isCorrect) {
                          bgColor = "bg-rose-600/40";
                        }
                        if (isSelected && !showResult) {
                          bgColor = "bg-rose-500/30 ring-rose-500 ring-2";
                        }
                        ...
                        return (
                          <button
                            ...
                            disabled={showResult}
                            key={optionIndex}
                            className={`w-full text-left p-2 rounded ${bgColor} 'hover:!bg-rose-500 dark:hover:!bg-rose-500/20 !duration-300 flex justify-between items-center`}
                            onClick={() => {
                              setKey(key + 1);
            ...
            {time !== 0 && quizData.length > 0 && (
              <Button
                type="button"
                className="w-full mt-2 !rounded-full !bg-gradient-to-r from-rose-800 to-rose-500 text-white pb-1"
                onClick={() => {
                  handleSubmitExam();
                  setEndExam(true);
  ...
  return (
    <>
      ...
      {loadingQuiz && quizData.length === 0 ? (
        ...
        <div className="grid items-center justify-center gap-5 p-5 rounded-xl bg-white dark:bg-gray-900">
          <div className="space-y-4">
            ...
            <h2 className="text-center text-base font-normal">
              এই কুইজে তোনার কাছে {quizData.length} মিনিট সময় থাকবে। প্রতিটি
              সঠিক উত্তরের জন্য +1 স্কোর পাবে। কুইজ শুরু করার পর সময় গননা শুরু
              হবে। সময় শেষ হলে কুইজ অটমেটিক সাবমিট হয়ে যাবে।
            </h2>
          </div>
          <Button
            type="button"
        ...
        <div className="bg-white shadow rounded-lg dark:bg-rose-900/30 p-4">
          <h2 className="text-xl font-semibold py-3 flex items-center gap-2">
            <span className="p-2 rounded-full bg-gradient-to-r text-rose-600 from-teal-600/20 to-rose-300/20">
              <Lightbulb />
            </span>
          ...
          <p className="text-center text-gray-500 dark:text-gray-100">
            Quiz is not available for this post.
          </p>
        </div>
      )}
