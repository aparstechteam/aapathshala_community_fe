...
export const SummaryComponent = (props: Props) => {
  ...
  async function handleReact(r: string) {
    try {
      const res = await axios.post(
        `${secondaryAPI}/api/post/${postId}/summary/react`,
        { reaction: r },
        {
      ...
      if (res.data) {
        ...
        toast({
          ...
          description:
            "আপনার মতামতের ভিত্তিতে আমরা এই বিষয়টি আরও ভালো করার চেষ্টা করব।",
          variant: "default",
        });
      }
    } catch (err) {
  ...
  return data.summary.length > 0 ? (
    <div className="grid bg-white shadow rounded-lg dark:bg-rose-900/30 p-4 gap-4 !text-gray-700 dark:!text-gray-200">
        ...
        {data.summary?.map((s, index) => (
          <div key={s.id}>
            <div className="text-base flex items-center gap-1 font-semibold dark:text-gray-100">
              <span>{index + 1}. </span>{" "}
              <AppMath key={index} formula={s.question} />
            </div>
      ...
      <div className="p-2 space-y-3 text-gray-700 dark:text-gray-100">
        <h2 className="font-medium text-base text-center">
          আলোচ্য বিষয়টি কি তুমি বুঝতে পেরেছো?
        </h2>
        <div className="flex items-center justify-center gap-3">
          <Button
            ...
            className={`w-20 !p-0 ring-[green]/20 text-rose-600 dark:ring-ash/50 ${
              reaction === "useful"
                ? "bg-rose-600/80 ring-0 text-white dark:bg-rose-600/10"
                : "ring-0 bg-rose-600/10 dark:bg-rose-600/10"
            }`}
            size="sm"
          >
            <span className="pt-0.5">হ্যাঁ</span> <ThumbsUp />
          </Button>
          <Button
            type="button"
            onClick={() => {
            ...
            className={`w-20 !p-0 text-red-600 dark:ring-ash/50 ${
              reaction === "not_useful"
                ? "bg-red-600/80 ring-0 text-white dark:bg-red-600/10"
                : "ring-0 bg-red-600/10 dark:bg-red-600/10"
            }`}
            size="sm"
          >
            <span className="pt-0.5">না</span> <ThumbsDown />
          </Button>
        </div>
      </div>
    </div>
  ) : loading ? (
    <div className="bg-white shadow rounded-lg dark:bg-rose-900/30 p-4"></div>
  ) : (
    <div className="bg-white shadow rounded-lg dark:bg-rose-900/30 p-4">
      ...
      <p className="text-center text-gray-500 dark:text-gray-100">
        Summary is not available for this post.
      </p>
    </div>
  );
};
