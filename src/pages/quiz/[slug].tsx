...
const ExamPage = () => {
    ...
    useEffect(() => {
        async function fetchExam() {
            try {
                if (!slug) return;
                const res = await axios.get(`${secondaryAPI}/api/exam/${slug}/questions`, {
                    headers: {
    ...
    const handleAnswer = (questionId: string, optionId: string) => {
        setAnswers((prev) => {
            const existingAnswerIndex = prev.findIndex((answer) => answer.q === questionId);
    ...
    async function submitAnswer() {
        ...
        try {
            setLoading(true)
            const response = await axios.post(`${secondaryAPI}/api/exam/${slug}/submit`, answers,
                {
                    headers: {
                ...
                <div className='w-full min-h-[calc(100vh-80px)] p-4'>
                    <div className='w-full max-w-5xl mx-auto h-full p-4 ring-1 ring-ash rounded-xl bg-white dark:bg-[#202127]'>
                            ...
                            {questions.map((question, index) => (
                                <div key={index} className='p-2'>
                                    <div className='mb-2 flex items-start gap-2'>
                                        ...
                                        <AppMath className='!w-full' formula={question?.question || ''} />
                                    </div>

                                    <div className='flex flex-col gap-2 px-2'>
                                        {question?.options?.map((option, index) => (
                                            <button type='button' onClick={() => handleAnswer(question?.id, option?.id)} key={index}
                                                className={cn('py-2 px-4 duration-300 transition-all rounded-lg ring-1 ring-ash text-start flex items-center gap-2',
                                                    answers.some((x) => x.q === question?.id && x.a === option?.id) && 'bg-gradient-to-r from-green-500 to-green-600 text-white')
                                                }>
                                                <span className='text-sm'>{options[index]}.</span>
                                                <AppMath className='!w-full' formula={option?.option || ''} />
                                            </button>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className='flex justify-center py-4'>
                            <Button className='w-full max-w-sm bg-gradient-to-r from-green-500 to-green-600 text-white' type='submit'
                                onClick={(e) => {
                                    e.preventDefault()
                                    submitAnswer()
                                }}>
