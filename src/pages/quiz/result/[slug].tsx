...
const QuizResult = () => {
    ...
    useEffect(() => {
        async function fetchResult() {
            try {
                if (!slug) return
                const response = await axios.get(`${secondaryAPI}/api/exam/${slug}/solution`, {
                    headers: {
                                        ...
                                        {question?.options?.map((option, index) => (
                                            <div key={index}
                                                className={cn('py-2 px-4 duration-300 transition-all rounded-lg ring-1 ring-ash text-start flex items-center gap-2',
                                                    option.is_correct && 'bg-green-500/30',
                                                    answers.some((x) => x.q === question?.id && x.a === option?.id) && !option.is_correct && 'bg-red-500/30'
                                                )}>
                                                <span className='text-sm'>{options[index]}.</span>
                                                <AppMath className='!w-full' formula={option?.option || ''} />
                                                {answers.some((x) => x.q === question?.id && x.a === option?.id) && !option.is_correct && (
                                                    <span className='text-xs text-end text-red-600'>
                                                        <X className='w-4 h-4' />
                                                    </span>
                                                )}
                                                {answers.some((x) => x.q === question?.id && x.a === option?.id) && option.is_correct && (
                                                    <span className='text-xs text-end text-green-600'>
